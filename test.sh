...
pid=$(pgrep -f "hysteria-linux-$amd$arm")
...
process_name="hysteria-linux-$amd$arm"
...
if [ -f "/root/hy3/hysteria-linux-$amd$arm" ]; then
  rm -f "/root/hy3/hysteria-linux-$amd$arm"
...
rm -r hysteria-linux-$amd$arm

wget -O hysteria-linux-$amd$arm https://github.com/apernet/hysteria/releases/download/app/v2.2.2/hysteria-linux-$amd$arm

chmod +x hysteria-linux-$amd$arm

nohup ./hysteria-linux-$amd$arm server &
...
if wget -O hysteria-linux-$amd$arm https://github.com/apernet/hysteria/releases/download/app/v2.2.2/hysteria-linux-$amd$arm; then
  chmod +x hysteria-linux-$amd$arm
else
...
if sudo setcap cap_net_bind_service=+ep hysteria-linux-$amd$arm; then
...
if nohup ./hysteria-linux-$amd$arm server & then
...
hysteria_executable="/root/hy3/hysteria-linux-$amd$arm"
